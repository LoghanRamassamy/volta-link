...
describe('Value Objects', () => {
  describe('OriginalUrl', () => {
    ...
    it('should throw error for non-http/https URL', () => {
      expect(() => OriginalUrl.create('ftp://google.com')).toThrow('URL must use HTTP or HTTPS protocol.');
    });
  });
  ...
  describe('UrlCode', () => {
    ...
    it('should throw error if length is too long', () => {
      expect(() => UrlCode.create('abcdefghijk')).toThrow('Code length must be between 3 and 10 characters.');
    });

    it('should throw error for special characters not allowed', () => {
      expect(() => UrlCode.create('abc$12')).toThrow('Code must contain only alphanumeric characters, hyphens, or underscores.');
    });
  });
  ...
  describe('ExpirationDate', () => {
    ...
    it('should throw error if date is in the past', () => {
      ...
      expect(() => ExpirationDate.create(pastDate, mockNow)).toThrow('Expiration date must be in the future.');
    });

    it('should throw error for invalid date formats', () => {
      expect(() => ExpirationDate.create('not-a-date', mockNow)).toThrow('Invalid expiration date format.');
    });
  });
});
